...
End Function
Function display_work_rules() 'displays a dialog showing the general work rules for SNAP, including a list of members that may be subject to rules
	...
	'Go through the member_array we just generated and check ages
		For each hh_member in enhanced_HH_member_array
			if hh_member.birthdate < sixteen_date AND hh_member.birthdate > fifty_nine_date Then hh_member.first_checkbox = True 'using first checkbox to store the users we need
		Next
